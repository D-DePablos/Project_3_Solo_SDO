...
lc = SkyCoord(*left_corner, frame=m.coordinate_frame)
rc = SkyCoord(*right_corner, frame=m.coordinate_frame)

fig = plt.figure()
m_sub = m.submap(lc, top_right=rc)
ax = fig.add_subplot(121, projection=m_sub)
m_sub.plot(axes=ax, norm=norm)
m_deconvolved_sub = m_deconvolved.submap(lc, top_right=rc)
